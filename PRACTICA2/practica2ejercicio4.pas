...
Procedure leerMaestro (var mae: maestro; var reg: producto);
begin
		if (not(eof(mae))) then
			read(mae, reg)
		else
...
Procedure leerDetalle (var det:detalle; var reg: sucursales);
begin
		if (not(eof(det))) then
			read(det, reg)
		else
...
begin
	...
	for i:= 1 to DF do begin
		...
		Str(i, num); //convierto el entero en un str
		name := ('Detalle' + num); // concateno
		assign(v[i], name);
		rewrite(v[i]);
		cargarDetalle(v[i]);
	end;
...
begin
	...
	while (reg.cod <> valorAlto) do begin
		...
		//el maestro se actualiza 1 vez x codigo
